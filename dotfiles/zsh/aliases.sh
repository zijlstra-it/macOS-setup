...
# specific aliases
alias unsetOS_='eval `set | grep -e OS_ -e MY_CLOUD | cut -d= -f1 | xargs -I {} echo "unset {}"`'
alias exportOS_='eval `~/bin/export-clouds-yaml` ; unset OS_CLOUD ; set | grep ^OS_ | grep -v OS_PASS'
alias resetEnv='unsetOS_ ; pyenv deactivate'
alias gco-dev='git checkout-dev'
alias gco-prod='git checkout-prod'
alias gco-main='git checkout-main'
alias code-new-window='code --new-window'
alias code-diff='code --new-window --diff'
